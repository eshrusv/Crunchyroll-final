...
MAX_THREADS          = 250
MAX_MASS_CHECK       = 15
MAX_BULK_LINES       = 1000000
EXECUTOR_MAX_WORKERS = 250
...
async def send_join_required(update,ctx):
    kb=InlineKeyboardMarkup([[InlineKeyboardButton("▸  Join Channel",url=CHANNEL_LINK,style="primary")],[InlineKeyboardButton("↺  I've Joined — Retry",callback_data="retry_join",style="success")]])
...
async def send_hit_result(bot, chat_id: int, report: str):
    """Send HIT result — with hit_media photo/video if set, otherwise plain text.
    Telegram caption limit = 1024 chars. If report exceeds it, send media first then text."""
    hit_media = mongo_get_media("hit_media")
    if hit_media:
        fid  = hit_media.get("file_id")
        mtype = hit_media.get("type")
        if len(report) <= 1024:
            # fits in caption — send together
            await send_media_with_caption(bot, chat_id, hit_media, caption=report)
        else:
            # too long — send media first, then details as separate message
            if mtype == "photo":
                await safe_send_photo(bot, chat_id, fid)
            elif mtype == "video":
                await safe_send_video(bot, chat_id, fid)
            await safe_send_message(bot, chat_id, report)
    else:
        await safe_send_message(bot, chat_id, report)

# ── MENU TEXT (kept short to fit 1024 char caption limit) ──────
def build_menu_text(name: str) -> str:
    """Telegram caption limit is 1024 chars — keep menu text short."""
    return (
        f"<b>◈  CRUNCHYROLL CHECKER</b>\n\n"
        f"<b>▸</b> Welcome, <b>{name}</b>\n\n"
        f"<b>▸</b> Fast · Reliable · Free\n"
        f"<b>▸</b> Select an option below\n\n"
        f"<i>{COPYRIGHT}</i>"
    )

def build_menu_text_full(name: str) -> str:
    """Full text version used when no media is set (no char limit)."""
    return (
...
async def start(update,ctx):
    ...
    kb_rows=[
        [InlineKeyboardButton("◉  Crunchyroll Checker",callback_data="checker_crunchyroll",style="primary")],
        [InlineKeyboardButton("◈  Proxy",callback_data="proxy_menu",style="primary"),
         InlineKeyboardButton("◧  Tools",callback_data="tools_menu",style="primary")],
        [InlineKeyboardButton("◑  My Profile",callback_data="my_profile",style="success")],
    ]
    if uid==OWNER_ID: kb_rows.append([InlineKeyboardButton("◆  Owner Panel",callback_data="owner_panel",style="danger")])
    ...
    if menu_media:
        # ── With media ──────────────────────────────────────────
        # Caption must be ≤ 1024 chars — use short version
        caption = build_menu_text(name)
        fid     = menu_media.get("file_id")
        mtype   = menu_media.get("type")

        if update.callback_query:
            q = update.callback_query
            prev_msg = q.message
            # If previous message is already a photo/video — edit its caption + keyboard
            # Otherwise delete and send fresh
            try:
                if prev_msg.photo or prev_msg.video:
                    # edit caption in place — no flash
                    if mtype == "photo" and prev_msg.photo:
                        await _tg_call(lambda: ctx.bot.edit_message_caption(
                            chat_id=prev_msg.chat_id, message_id=prev_msg.message_id,
                            caption=caption, parse_mode=ParseMode.HTML, reply_markup=kb), prev_msg.chat_id)
                        return
                    elif mtype == "video" and prev_msg.video:
                        await _tg_call(lambda: ctx.bot.edit_message_caption(
                            chat_id=prev_msg.chat_id, message_id=prev_msg.message_id,
                            caption=caption, parse_mode=ParseMode.HTML, reply_markup=kb), prev_msg.chat_id)
                        return
            except: pass
            # Different media type or text message — delete and resend
            try: await safe_delete_message(ctx.bot, prev_msg.chat_id, prev_msg.message_id)
            except: pass
        # Send fresh media message
        if mtype == "photo":
            await safe_send_photo(ctx.bot, update.effective_chat.id, fid,
                                  caption=caption, parse_mode=ParseMode.HTML, reply_markup=kb)
        elif mtype == "video":
            await safe_send_video(ctx.bot, update.effective_chat.id, fid,
                                  caption=caption, parse_mode=ParseMode.HTML, reply_markup=kb)
        else:
...
async def owner_panel_callback(update,ctx):
    ...
    kb=InlineKeyboardMarkup([[InlineKeyboardButton("◂  Back",callback_data="back_to_main",style="primary")]])
...
async def checker_crunchyroll_callback(update,ctx):
    ...
    kb=InlineKeyboardMarkup([
        [InlineKeyboardButton("◎  Single Check  (/check)",  callback_data="cr_cmd_check",  style="primary")],
        [InlineKeyboardButton("▦  Mass Check    (/mcheck)", callback_data="cr_cmd_mass",   style="primary")],
        [InlineKeyboardButton("▤  Bulk Check    (/bulk)",   callback_data="cr_cmd_bulk",   style="primary")],
        [InlineKeyboardButton("■  Stop Checking (/stop)",   callback_data="cr_cmd_stop",   style="danger")],
        [InlineKeyboardButton("◂  Back",                    callback_data="back_to_main",  style="primary")],
    ])
...
async def proxy_menu_callback(update,ctx):
    ...
    kb=InlineKeyboardMarkup([
        [InlineKeyboardButton("◈  Test Proxy",callback_data="test_proxy",style="success")],
        [InlineKeyboardButton("◂  Back",callback_data="back_to_main",style="primary")],
    ])
    await q.edit_message_text(msg,reply_markup=kb,parse_mode=ParseMode.HTML)
...
async def myproxy_cmd(update,ctx):
    ...
    kb=InlineKeyboardMarkup([[InlineKeyboardButton("◈  Test Proxy",callback_data=f"test_myproxy_{uid}",style="success")],[InlineKeyboardButton("◂  Back",callback_data="back_to_main",style="primary")]])
    if update.callback_query: await update.callback_query.edit_message_text(msg,reply_markup=kb,parse_mode=ParseMode.HTML)
    else: await _reply(update,msg,reply_markup=kb)
...
async def auto_detect_combos(update,ctx):
    ...
    kb=InlineKeyboardMarkup([[InlineKeyboardButton(f"▶  Check {count} Combos",callback_data=f"auto_bulk_{callback_id}",style="success")]])
...
def main():
    ...
    logger.info(">>> Bot v1.7.0 (MongoDB + Media + Colored Buttons) starting ...")
    while True:
